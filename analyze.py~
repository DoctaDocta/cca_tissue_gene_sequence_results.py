...
con = lite.connect('/Users/elenka/Documents/larriscku/geneSequenceResults.db'); #initilazing output file
...
manifestFile = "/Users/elenka/Documents/larriscku/file_manifest.txt"; #this is the manifest file path. it has barcode names and filenames.
pathToDataFiles = "/Users/elenka/Documents/larriscku/RNASeqV2/UNC__IlluminaHiSeq_RNASeqV2/Level_3/";#where the files are,will append to filenames from manifest.
...
########################################################
with open(tissue_type_file, 'rt') as f:
...
#functions to add attribute to the object....
	...
	def add_genes_norms(self,filename): #read in 1 of 3 files, get information to execute immediately..
		...
		with open(output_genes, 'w') as p: #this is to write formatted output
			...
			with open(path, 'rt') as f: #this is to read the genes_norms file
	...
	def get_isoforms_norms(self,filename): #read in 1 of 3 files, store information for later...
		...
		#print("\tAdded filepath to TissueSample.")
		with open(path, 'rt') as f: #use the file we read in to get all its necessary contents
	...
	def get_genes_results(self,filename): #read in 1 of 3 files, store information for later...
		...
		#print("\tAdded filepath to TissueSample.")
		with open(path, 'rt') as f: #this is using the csv library to read the file into python
	...
	def add_isoforms_ids(self):
		...
		with open(output_isoforms, 'w') as p:
			writer = csv.writer(p, delimiter='\t')
			for key, val in self.genes_results.items():
				for x in val:
